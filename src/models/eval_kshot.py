...
def eval_kshot(args, run):

    for model_run in os.listdir(args.language_model):
        for num_negatives in ["none", "one", "two", "all"]:
            ...
            if num_negatives == "none":
                kshot_negatives = "0"
            elif num_negatives == "one":
                kshot_negatives = "1"
            elif num_negatives == "two":
                kshot_negatives = "2"
            elif num_negatives == "all":
                kshot_negatives = "A"
            else:
                raise ValueError("unknown negatives.")

            if "0" in args.language_model:
                pretrain_negatives = "0"
            elif "1" in args.language_model:
                pretrain_negatives = "1"
            elif "2" in args.language_model:
                pretrain_negatives = "2"
            else:
                raise ValueError("unknown negatives.")

            exp = f"tag_extension_full/{pretrain_negatives}neg{kshot_negatives}neg"
